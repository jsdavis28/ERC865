...
contract('ERC865BasicToken', function ([j, k, feeAccount, delegate]) {
    ...
    //Signature of testMsg1, signed by sender
    const testSig1 =
    ...
    //Signature of testMsg2, signed by sender
    const testSig2 =
    ...
    //Signature of testMsg3, signed by sender
    const testSig3 =
    ...
    beforeEach(async function () {
        this.token = await ERC865BasicTokenMock.new(sender, 1000, feeAccount, testSig2);
    });
    ...
    describe('transfer presigned', function () {
                ...
                describe('when the sender address is recovered', function () {
                    describe('when the sender has enough balance', function () {
                        it('transfers the requested amount', async function () {
                            await this.token.
                                _transferPreSigned(testSig1, sender, to, amount, fee, nonce, { from: _delegate });
                    ...
                    describe('when the sender does not have enough balance', function () {
                        it('reverts', async function () {
                            await assertRevert(this.token.
                                _transferPreSigned(testSig3, sender, to, 1000, 2, 3, { from: _delegate }));
                        });
                    });
                });

                describe('when the sender address is not recovered', function () {
                    const wrong = to;

                    it('reverts', async function () {
                        await assertRevert(this.token.
                            _transferPreSigned(testSig1, wrong, to, amount, fee, nonce, { from: _delegate }));
                    });
                });
            });

            describe('when the signature is not original', function () {
                it('reverts', async function () {
                    await assertRevert(this.token.
                        _transferPreSigned(testSig2, sender, to, 10, 2, 2, {from: _delegate }));
                });
            });
        });
        ...
        describe('when the recipient is the zero address', function () {
            it('reverts', async function () {
                await assertRevert(this.token.
                    _transferPreSigned(testSig1, sender, ZERO_ADDRESS, amount, fee, nonce, { from: _delegate }));
            });
        });
    });
});
